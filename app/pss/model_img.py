                ...
                if return_DC:
                    if instance[2] == "FirstPage" or prevPageClass == "FirstPage":
                        data_instances.append(
                            [instance[1], instance[2], instance[4], prevPageId, prevPageText, instance[3]])
                else:
...
def predict(model, data, img_dim, prev_page_generator=False, batch_size=32):
    if prev_page_generator:
        y_predict = np.round(
            model.predict_generator(ImageFeatureGenerator(data, img_dim, prevpage=True, batch_size=batch_size)))
    else:
        y_predict = np.round(
            model.predict_generator(ImageFeatureGenerator(data, img_dim, prevpage=False, batch_size=batch_size)))
    return y_predict
